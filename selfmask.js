...
style: cytoscape.stylesheet()
    .selector('node')
	  ...
	  elements: {
	    populations: [
	      ...
	      selectable: true,
	        grabbable: false}, // NB no group specified
	...
	HexesSelfmaskPlugin.prototype.buildForm = function(populationsCount,
		...
		hexesTable.append(createRow("Selfmask"));
		hexesTable.append(createRow("Othermask"));
	}
	...
	HexesSelfmaskPlugin.prototype.fillForm = function(populationsCount,
		...
		$("input.hexes").each(function(key, val) {
			...
			if ($input.data('rowName') == 'Othermask') {
				$input.val(intToHex(masks.othermasks[num]));
			} else if ($input.data('rowName') == 'Selfmask') {
				$input.val(intToHex(masks.selfmasks[num]));
			}
		});
	}
